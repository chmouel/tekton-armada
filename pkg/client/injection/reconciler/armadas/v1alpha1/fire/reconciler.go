...
import (
	...
	armadasv1alpha1 "github.com/chmouel/armadas/pkg/client/listers/armadas/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.Fire.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Fire. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Fire) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Fire.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Fire. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Fire) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Fire if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Fire.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Fire) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Fire) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.Fire resources.
type reconcilerImpl struct {
	...
	Lister armadasv1alpha1.FireLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister armadasv1alpha1.FireLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Fires(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.Fire, desired *v1alpha1.Fire) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.GithubV1alpha1().Fires(desired.Namespace)
		...
		updater := r.Client.GithubV1alpha1().Fires(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Fire, desiredFinalizers sets.String) (*v1alpha1.Fire, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.GithubV1alpha1().Fires(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Fire) (*v1alpha1.Fire, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Fire, reconcileEvent reconciler.Event) (*v1alpha1.Fire, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
